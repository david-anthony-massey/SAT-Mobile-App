...
function S4QuestionRow(props) {
  ...
  if (isBlank) {
    ...
    AsyncStorage.setItem(`S4${props.qNumber}`, `${code}`);
  } else {
        ...
        <Checkbox
          ...
          onCheck={() => {
            if (initialCode != -1) {
              ...
              AsyncStorage.setItem(`S4${props.qNumber}`, `-2`).then(() => {
                setInitialCode(-2);
              });
            }
          }}
      ...
      <RadioForm
        ...
        onPress={value => {
          if (isGuess) {
            ...
            AsyncStorage.setItem(`S4${props.qNumber}`, `${value + 4}`).then(
              () => {
                setInitialCode(value + 4);
              }
            );
          } else {
            AsyncStorage.setItem(`S4${props.qNumber}`, `${value}`).then(() => {
...
export default memo(S4QuestionRow);
